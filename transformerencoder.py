...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, mask=None, relative_pos=None):
        ...
        self.last_attn_weights = attn_weights  # Store the attention weights
...
class PolymerEncoder(nn.Module):
    def __init__(
        self, 
        vocab_size, 
        d_model=256,       # Match reduced size
        n_layers=6,        # Match reduced size
        n_heads=8,         # Match reduced size
        d_ff=1024,        # Match reduced size
        dropout=0.1,
        max_seq_length=150
    ):
        super().__init__()
    ...
    def forward(self, src: torch.Tensor, src_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i, layer in enumerate(self.layers):
            if self.training:
                x = checkpoint(  # Remove torch.utils.checkpoint
                    layer,
                    x,
                    attention_mask,
                    self.rel_pos_bias
                ...
                if i == len(self.layers) - 1:  # Store weights from last layer
                    attention_weights = layer.last_attn_weights
